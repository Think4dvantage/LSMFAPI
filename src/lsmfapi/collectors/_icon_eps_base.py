"""Shared base for the ICON-CH1-EPS and ICON-CH2-EPS collectors.

CH1 and CH2 differ only in a handful of parameters (collection id, horizon range, member
count, ref-dt guard, grid constants filename, and whether an accumulation baseline needs a
shadow fetch). Everything else — STAC search, GRIB parsing, download/extract/deaccumulate/
build-stations/build-grid pipeline — is identical and lives here.

Grid state (KD-tree, pre-sampled indices, model-level heights) stays as module-level
singletons in each of icon_ch1_eps.py / icon_ch2_eps.py rather than here, because CH1 and
CH2 build genuinely different grids (1.1 km vs 2.2 km) and existing tests reset them via
direct module-attribute access (e.g. `ch1_mod._GRID_TREE = None`). `_ensure_grid` and
`collect_grid` therefore stay defined per-subclass; this base class reaches that state
through small per-subclass accessor methods (`_grid_tree`, `_grid_level_heights`) instead
...
# Both CH1 and CH2 fetch the same three pressure-level variables — verified identical and
# never diverging (formerly two separately-declared identical lists, PRESSURE_VARS /
# CH1_PRESSURE_VARS).
...
# before this refactor, since CH2 imported the same _read_grib2_eccodes function (and thus
# closed over the same set) from icon_ch1_eps.py rather than having its own copy.
...
# ---------- Shared grid helpers (used by both CH1 and CH2 collect_grid) ----------
...
# ---------- Shared collector base ----------

class IconEpsCollectorBase(BaseCollector):
    """Shared STAC/GRIB/ensemble pipeline for the ICON-*-EPS collectors.

    Subclasses (IconCh1EpsCollector, IconCh2EpsCollector) provide the parameters that
    differ — collection id, horizon range, telemetry tag, model name, accumulation
    baseline, ref-dt guard, grid constants filename — as class attributes, plus a handful
    of small accessor methods (`_cfg`, `_compute_ref_dt`, `_grid_tree`,
    `_grid_level_heights`) that reach into that subclass's own module-level state. This
    indirection exists so config/HORIZONS monkeypatching in tests (which patches the
    subclass *module*, e.g. `icon_ch1_eps.get_config`) keeps working, and so the grid
    KD-tree/sample-index singletons stay separate per model as today.
    """
    ...
    def HORIZONS(self) -> list[int]:
        """The collector's horizon list. Implemented as a property (not a plain class
        attribute) forwarding to the subclass module's own HORIZONS global, so that
        `monkeypatch.setattr(ch1_mod, "HORIZONS", ...)` in tests still takes effect."""
        raise NotImplementedError

    @abstractmethod
    def _cfg(self):
        """Return get_config() — implemented per subclass so it resolves via that
        subclass's own module (tests monkeypatch `<module>.get_config`)."""
        raise NotImplementedError

    @abstractmethod
    def _compute_ref_dt(self) -> datetime:
        """Return the latest reference datetime — delegates to the subclass module's
        standalone _latest_ref_dt / _latest_ref_dt_ch2 function."""
        raise NotImplementedError

    @abstractmethod
    def _grid_tree(self):
        """Return the subclass module's _GRID_TREE singleton (or None before _ensure_grid)."""
        raise NotImplementedError

    @abstractmethod
    def _grid_level_heights(self):
        """Return the subclass module's _GRID_LEVEL_HEIGHTS singleton (or None)."""
        raise NotImplementedError

    @abstractmethod
    async def _ensure_grid(self, tmpdir: Path) -> None:
        """Build (once per process) the KD-tree, grid-sample indices and model-level
        heights for this model, stored as module-level singletons in the subclass
        module. Left per-subclass: CH1/CH2 build genuinely different grids and existing
        tests reset these singletons via direct module-attribute access."""
        raise NotImplementedError

    @abstractmethod
    def collect_grid(self, ref_dt: datetime, tmpdir: Path) -> None:
        """Build and cache the wind + thermal grids from GRIBs left in tmpdir. Left
        per-subclass for the same reason as _ensure_grid."""
        raise NotImplementedError
    ...
    async def _fetch_step(
        ...
        async with semaphore:
            try:
                url = await _search_item_url(
                    client, cfg.meteoswiss.stac_base_url, self.COLLECTION,
                    ref_dt, variable, horizon_h,
                )
            except Exception as exc:
                logger.error("%s STAC search error %s h=%d: %s", tag, variable, horizon_h, exc)
                _telemetry.record_download_error(self.MODEL_TAG, variable, horizon_h, f"STAC search: {exc}")
                return None

            if url is None:
                logger.warning(
                    "%s STAC: no asset found for %s h=%d — variable may not be published",
                    tag, variable, horizon_h,
                )
                return None

            dest = tmpdir / f"{variable}_{horizon_h:03d}.grib2"
            if dest.exists() and dest.stat().st_size > 1024:
                logger.debug("%s GRIB cache hit: %s h=%d", tag, variable, horizon_h)
            else:
                try:
                    await self.download(url, str(dest))
                except Exception as exc:
                    logger.error("%s download failed %s h=%d: %s", tag, variable, horizon_h, exc)
                    _telemetry.record_download_error(self.MODEL_TAG, variable, horizon_h, f"Download: {exc}")
                    return None
        ...
        try:
            ...
            if arr is None:
                logger.warning("%s eccodes returned None for %s h=%d", tag, variable, horizon_h)
                return None
            logger.debug("%s data %s h=%d shape=%s", tag, variable, horizon_h, arr.shape)
            return arr
        except Exception as exc:
            logger.error(
                "%s eccodes read failed %s h=%d: %s — removing cached file",
                tag, variable, horizon_h, exc,
            )
            dest.unlink(missing_ok=True)
            _telemetry.record_download_error(self.MODEL_TAG, variable, horizon_h, f"eccodes: {exc}")
            return None
        finally:
    ...
    async def collect(self) -> None:  # noqa: C901
        ...
        with grib_run_dir(self.MODEL_TAG, ref_dt, self._cfg().grib_cache_dir) as tmpdir:
            ...
            if not stations:
                logger.warning("No stations returned; skipping %s collection", tag)
                return
            ...
            _, station_flat_indices = self._grid_tree().query(station_coords)
            ...
            async with httpx.AsyncClient(timeout=300) as client:
                ...
                u0_url = await _search_item_url(
                    client, cfg.meteoswiss.stac_base_url, self.COLLECTION, ref_dt, "U", horizons[0]
                )
                if u0_url:
                    dest = tmpdir / self.U_PROBE_FILENAME
                    try:
                        ...
                        logger.warning("%s pressure level probe failed: %s", tag, exc)
                    finally:
                        dest.unlink(missing_ok=True)

                if pres_level_nums is None:
                    # Wording differs by model in the original code (CH1 names the
                    # collection, CH2 doesn't) — preserved exactly rather than unified,
                    # since the difference is more than a CH1/CH2 tag substitution.
                    if self.MODEL_TAG == "ch1":
                        logger.info("Model-level probe empty for %s — assuming 80 levels", self.COLLECTION)
                    else:
                ...
                _cs.mark_running(self.MODEL_TAG, ref_dt, n_surf + n_pres)
                ...
                async def fetch(var: str, h: int) -> np.ndarray | None:
                    ...
                    try:
                        ...
                        _cs.mark_progress(self.MODEL_TAG, done, ok)
                        if done % 20 == 0 or done == total:
                            logger.info("%s %d/%d ok=%d (%s h=%d)", tag, done, total, ok, var, h)

                # Shadow-fetch the accumulation baseline horizon for each accumulated
                # variable — only when this model needs one (CH2). CH1 starts at h=0
                # where the true accumulated baseline is genuinely zero.
                prior_tasks: dict[str, asyncio.Task] = {}
                if self.ACCUM_PRIOR_H is not None:
                    prior_tasks = {
                        var: asyncio.ensure_future(
                            self._fetch_step(
                                semaphore, client, ref_dt, var, self.ACCUM_PRIOR_H,
            ...
            logger.info("%s surface fetch: %d/%d tasks returned data", tag, n_surf_ok, n_surf_total)
            ...
            def _get_prior(var: str) -> np.ndarray:
                ...
                if t is None or t.cancelled():
                    logger.warning(
                        "%s deaccumulation baseline missing for %s (h%d fetch missing/cancelled) "
                        "— h%d delta will be null, not a wrong number",
                        tag, var, self.ACCUM_PRIOR_H, horizons[0],
                    )
                    _telemetry.record_download_error(self.MODEL_TAG, var, self.ACCUM_PRIOR_H, "prior fetch missing/cancelled")
                    return _prior_fallback
                try:
                    r = t.result()
                except Exception as exc:
                    logger.warning("%s deaccumulation baseline fetch failed for %s: %s", tag, var, exc)
                    _telemetry.record_download_error(self.MODEL_TAG, var, self.ACCUM_PRIOR_H, f"prior fetch failed: {exc}")
                    return _prior_fallback
                if isinstance(r, np.ndarray) and r.ndim == 3:
                    r = r[:, -1, :]
                if isinstance(r, np.ndarray) and r.shape == _prior_fallback.shape:
                    return r
                logger.warning("%s deaccumulation baseline for %s has an unexpected shape — using null fallback", tag, var)
                _telemetry.record_download_error(self.MODEL_TAG, var, self.ACCUM_PRIOR_H, "prior fetch shape mismatch")
                return _prior_fallback

            def deaccum(arr: np.ndarray, var: str) -> np.ndarray:
                if self.ACCUM_PRIOR_H is None:
            ...
            if level_heights is not None and u_pl.shape[2] == level_heights.shape[0]:
                ...
                logger.warning(
                    "%s altitude winds: no level heights or level-count mismatch "
            ...
            def _build_and_cache_stations() -> None:
                ...
                for s_idx, station in enumerate(stations):
                    ...
                    logger.info("%s cached forecast for %s (%d hours)", tag, station_id, len(forecast_list))

            await asyncio.to_thread(_build_and_cache_stations)

            # Grid collection reads the U/V GRIBs kept in tmpdir. Off the event loop:
            # it parses hundreds of GRIBs (minutes for CH1, longer for CH2) and would
            # otherwise block uvicorn for that whole window — /health included. eccodes
            # (via cffi) and numpy release the GIL, so the loop keeps serving while this runs.
            try:
                await asyncio.to_thread(self.collect_grid, ref_dt, tmpdir)
            except Exception as exc:
                logger.exception("%s grid collection failed — station data unaffected", tag)
                ...
                _telemetry.record_download_error(self.MODEL_TAG, "grid", 0, str(exc))

        logger.info("%s.collect() complete — %d stations", self.__class__.__name__, len(stations))
