...
| v0.3.31 | Tech-debt remediation P2-5 (landed alone, not batched, per the plan): `numpy = "^1.26"` relaxed to `">=1.26,<3"` — no dependency actually required `<2.0`, the ceiling was purely Poetry's caret on a CalVer-adjacent pin, and it's why `poetry install` has never worked on this dev box (1.26.4 has no cp313 wheel) despite `python = "^3.11"` (i.e. `<4.0`) claiming broader support. Regenerated `poetry.lock` the same verified way as P2-4/P3-2 — Poetry's resolver kept the exact same `numpy==1.26.4` pin (it doesn't eagerly upgrade an existing lock), so **no runtime behaviour changes today**; this only removes the artificial ceiling for whenever a numpy 2.x bump is actually proposed, at which point the integration test is what has to prove it, not this commit |
| v0.3.32 | Tech-debt remediation P1-10: STAC search now uses a closed reference-datetime interval (`ref_dt/ref_dt`, was open-ended `ref_dt/..`) so a newer run that appears mid-collection (a run takes 1.5–2h; MeteoSwiss publishes continuously) can no longer silently match and stitch two different model runs into one forecast. Also logs a WARNING if a search returns more than one asset instead of silently picking `next(iter(...))` with no signal |
| v0.3.33 | Tech-debt remediation P1-11: two silent-fallback bugs fixed. CH2's `_get_prior` (deaccumulation baseline) fell back to `np.zeros` (misnamed `nan_prior`) on any h33-fetch failure with no log/telemetry — a silent full-34h-accumulation-as-1h-rate (~34× too high for precipitation); now logs WARNING + records telemetry and falls back to NaN (renamed `_prior_fallback`) so a failure surfaces as `null`, not a plausible-looking wrong number. CH1's thermal-grid solar/sunshine baseline (`prev_aswdir`/`prev_aswdifd`/`prev_dursun`) only advanced inside the success branch, so a single missing horizon made the *next* successful step silently difference against a stale 2-hour-old baseline (double-counted radiation) — now an explicit `*_baseline_ok` flag invalidates the baseline on any gap, so the following step is also null rather than wrong, matching the station path's existing fail-safe behaviour. Deleted the stale `features.md` "Known Issues" entry claiming `sunshine_minutes` is wrong on CH2's first step — `ACCUM_PRIOR_H` already handles it correctly |
| v0.3.34 | Tech-debt remediation P3-9: silent failure paths fixed. `_read`'s missing-file case (thermal-grid builder) and CH2's missing-STAC-asset case (CH1 already warned) now log WARNING instead of returning `None` silently. Grid-collection failures now also call `telemetry.record_download_error` (not just `logger.exception`), so a run with no grids is visible on the dashboard instead of reporting fully successful — `collection_state.mark_failed` deliberately *not* used, since stations did succeed and that would mislabel the whole run. `_eccodes_get`'s `perturbationNumber` fallback now counts occurrences per file and warns if more than one message defaults (exactly one — the control run — is expected; more signals members silently collapsing onto the same row). `_deaccumulate`'s `prepend=arr[:1,:]*0` replaced with `np.zeros_like(...)` — the old idiom turns `NaN*0` into `NaN`, silently breaking the intended zero baseline. Left `_fetch_step`'s unlink-on-parse-failure alone (🔎 in the plan) — it's consistent with this project's established corrupt-GRIB-self-deletes-so-it-re-downloads pattern, and guessing wrong here risks a corrupt file being retried forever instead |
| v0.3.35 | Tech-debt remediation P0-4.2 (partial — see RESUME for the full scope decision): `W`'s ~1.8 GB/horizon files are now deleted by `_fetch_step` immediately after their single station-level read (`delete_after_read=True`), instead of accumulating on disk for every horizon until a bulk-delete after the whole download phase completes. Cuts roughly 1/3 off the CH1/CH2 disk peak (W was never re-read by the grid build, unlike U/V). **U/V's contribution to the peak is intentionally not addressed here** — doing so safely requires interleaving grid computation with the download phase (bounded memory), not the naive "stash every horizon's grid array until later" approach the plan's literal text suggests, which would trade the disk-OOM for a ~26 GB memory-OOM invisible to CI's reduced-horizon integration test. That deeper rewrite is deferred |
| v0.3.36 | Tech-debt remediation P1-5: the remaining event-loop-blocking calls from v0.3.5's own follow-up list are now off the loop. `_fetch_step`'s `_read_grib2_eccodes` call (once per file — 782 CH1 / 2006 CH2 files per run, each a two-pass full read) now runs via `asyncio.to_thread`. The per-station stats loop (~8.7k `StationForecastHour` + ~78k `AltitudeWindLevel` Pydantic objects and ~296k `compute_stats` calls for CH2) is now a nested `_build_and_cache_stations()` function run via `asyncio.to_thread` instead of executing inline inside `async def collect()` — same shared-state rule as the v0.3.5 grid-build fix (only the `set_station_forecast`/`set_station_altitude_winds` calls touch shared state; everything else is built fresh per call) |
| v0.3.37 | Tech-debt remediation P3-5 — the largest change in the plan: extracted `IconEpsCollectorBase` into a new `collectors/_icon_eps_base.py` (STAC search, eccodes readers, grid-cache builders, and the full `_fetch_step`/`collect()` orchestration — 81% of CH1/CH2 was identical code). `icon_ch1_eps.py` (1227→209 lines) and `icon_ch2_eps.py` (585→188 lines) are now thin subclasses parameterized by `COLLECTION`/`MODEL_TAG`/`MODEL_NAME`/`ACCUM_PRIOR_H`/`REF_DT_GUARD_HOURS`/`GRID_CONSTANTS_PREFIX`; `HORIZONS` stays a per-subclass `@property` (not a plain attribute) and grid KD-tree/height singletons stay per-subclass module globals, both specifically so existing test monkeypatching (`ch1_mod.HORIZONS = ...`, `ch1_mod._GRID_TREE = None`) keeps working unchanged. Delegated to a subagent with an exhaustive brief, then independently re-verified every line of the ~1250-line base module and both subclasses by hand against the pre-refactor git history (confirmed the CH1/CH2 `deaccum` closures are byte-identical to before) and every monkeypatch path in all 3 test files |
